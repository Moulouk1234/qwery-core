...
export function ChartColorEditor({
  ...
  const computeDisplayColors = useCallback((colorArray: string[]) => {
    ...
    if (limited.length < colorCount) {
      ...
      return [
        ...limited,
        ...defaultColors.slice(limited.length, colorCount),
      ];
    }
    return limited;
  }, [colorCount]);
  
  const [localColors, setLocalColors] = useState<string[]>(() => computeDisplayColors(colors));
      ...
      <PopoverContent className="w-80 p-0" align="start">
        <Card className="border-0 shadow-none">
          ...
          <CardContent className="space-y-4">
            {localColors.slice(0, colorCount).map((color: string, index: number) => (
              <div key={index} className="space-y-2">
                <Label htmlFor={`color-${index}`} className="text-xs font-medium">
                  {colorCount === 1 ? 'Color' : `Color ${index + 1}`}
                </Label>
                <div className="flex items-center gap-2">
                  <div className="relative group">
                    <div
                      className="h-10 w-10 rounded-md border-2 border-border shadow-sm cursor-pointer transition-all hover:scale-105 hover:shadow-md"
